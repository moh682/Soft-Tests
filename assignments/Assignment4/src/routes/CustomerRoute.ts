...
route.get('/customer/create', async (req, res, next) => {
...
route.post('/customer/find', async (req, res, next) => {
    const cpr = req.body.cpr;

    try {
        const cm = new CustomerMapper();
        const foundCustomer = await cm.getByNumber(cpr);
        return res.status(200).send('Found customer: ' + foundCustomer);
    } catch (error) {
        return res.status(500).send(error);
    }
});

route.post('/customer/delete', async (req, res, next) => {
    const cpr = req.body.cpr;
